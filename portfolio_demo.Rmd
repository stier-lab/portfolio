...
where $T$ is the 'total' (the whole metapopulation), and $\sigma_i^2$ are the individual population variances. For this index, 0 represents perfect asynchrony and 1 represents perfect synchrony.
...
```
You can tell from this value that the populations are pretty synchronous, which makes sense given the predicted vs. observed variances in the plot above. If you want to dig deeper into whether certain species are synchronous or asynchronous with each other, you can use more sophisticated time series models like MARSS, which accounts for autocorrelation and can include things like environmental drivers. More for a later discussion! 
